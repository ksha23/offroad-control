...
architectures: on a 432-run planner-only sweep the pre-retrain rig advantage
narrows to a static-only effect (rig-static 0.090 vs.\ vehicle-static 0.116~m;
the rate variants tie within noise). The deployed rate model is, in fact, the
\emph{worst} of the four on per-tick force RMSE yet the most reliable in closed
loop: its smoother, lower-amplitude $F_y$ trace under-predicts transient peaks
but avoids a catastrophic failure mode the static checkpoint exhibits, which is
why it is the deployed default.
