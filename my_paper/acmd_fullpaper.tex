...
wheel-speed signals; and an asymmetric throttle disturbance observer reduces the
...
is Chrono::Vehicle from the open-source Project Chrono multi-physics engine,
driven by a synthetic 5G latency trace on the command and camera channels. Across repeatable simulation experiments the
...
four share one Chrono::Vehicle plant (Project Chrono~\cite{Tasora16,Serban19}),
one message stream, and one benchmarking harness, so the main comparisons run on
one matrix. The contribution is the \emph{integration}: evaluating these layers
...
Chrono::Vehicle, the ground-vehicle module of the open-source Project Chrono
multi-physics engine~\cite{Tasora16,Serban19}, simulates an HMMWV on an
SCM~\cite{Krenn09} patch; the human-in-the-loop evaluation is inspired by
Chrono::HIL~\cite{Zhou23}. The simulation and controller run as separate
processes and exchange messages over ROS~2 topics through Chrono::ROS, so any
node that
...
The model exports as a CasADi expression through which the acados solver
auto-differentiates at every horizon stage. The deployed model is trained on
$\sim$1.85\,M tuples from 800 closed-loop HMMWV runs whose soil parameters are
Latin-hypercube-sampled over a broad Bekker--Mohr box that \emph{encloses} the
clay, dirt and sand presets as interior points, so accuracy on those terrains is
interpolation and generalisation is measured on held-out slices.

Table~\ref{tab:tire} reports the live-estimator benchmark (1620 closed-loop
runs) against Pacejka and TMeasy baselines calibrated to the SCM plant's mean
lateral friction ($\mu=0.42$). The neural model conditioned on the matched
terrain preset is a near-oracle reference; with the online estimator recovering
the soil online it nearly matches that row (median 0.177 vs.\ 0.174~m) while
requiring no prior terrain knowledge, and both cut median RMS crosstrack error
by 39--41\% versus the analytical baselines. The gain is concentrated in the
off-nominal cells (dirt, higher speed) where the fixed force laws lose grip
(Fig.~\ref{fig:cte}); on benign cells the medians are comparable. A controlled
retrain confirms the whole-vehicle model is the right default once its training
data spans the same operating box as the rig; the longitudinal channel retains
a terrain-dependent motion-resistance bias that the disturbance observer
(\S\ref{sec:speed}) absorbs online rather than a tire-model feature deficit.

\begin{table}[t]
  \begin{center}
    \tabcaption{Live-estimator tire-model benchmark (median RMS crosstrack
      error, 1620 runs). Both neural rows reduce error 39--55\% versus the
      SCM-calibrated analytical baselines.}
    \vspace{1mm}
    \small
    \begin{tabular}{lccc}
      \toprule
      ...
      NN rate-MLP $+$ estimator & 0.17 & $-41\%$ & $-39\%$\\
      \bottomrule
    \end{tabular}
    \label{tab:tire}
  \end{center}
...
A controlled retrain isolates the rig-versus-whole-vehicle question with matched
architectures (432-run planner-only sweep). Once architecture and training
terrain are matched, the two generations track within noise: mean RMS crosstrack
error spans just 0.090--0.116~m across all six surrogates, with the rig
checkpoints marginally best (rig-static 0.090~m) and the deployed whole-vehicle
rate model mid-pack (0.105~m). The whole-vehicle model is therefore deployed not
because it wins this canonical-terrain sweep but because it is trained on the
operating-point distribution the controller actually visits and generalises to
off-manifold soils; within that generation, rate augmentation mainly reduces the
worst-case tail relative to the static variant (max CTE 0.30 vs.\ 0.33~m, with a
markedly smaller spread) rather than improving mean accuracy.

An open-loop check confirms the NMPC's internal predictions match the plant
(Fig.~\ref{fig:pred}): heading drift stays below 0.15~rad over the 4~s horizon on
every terrain, while the kinematic longitudinal model over-predicts speed on firm
soil and under-predicts on soft dirt. A feature-headroom study
(Table~\ref{tab:feat}) locates this at the model level: supplying the full causal
dynamic state improves front-axle $F_y$ by 14\%, but the longitudinal channel
sits at its feature floor ($\le$3\%) and rear-axle $F_y$ is a structural floor
(within 4\% even when handed oracle sinkage). The soft-soil speed bias is thus a
dynamics-level motion-resistance term the disturbance observer absorbs online
(\S\ref{sec:speed}), not a learnable tire feature.

\begin{table}[t]
  \begin{center}
    \tabcaption{Per-axle held-out force RMSE (N) as causal, MPC-available dynamic
      features are added to the deployed rate surrogate. Front $F_y$ improves
      14\%; $F_x$ and rear $F_y$ sit at their feature floors.}
    \vspace{1mm}
    \small
    \begin{tabular}{lcccc}
      \toprule
...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.98\textwidth]{rollout_prediction_validation.png}
    \figcaption{Open-loop NMPC prediction vs.\ the Chrono plant (firm sand). The
      predicted horizons trace the path geometry and heading accurately, but the
      over-predicted longitudinal speed carries each predicted endpoint ahead of
      the vehicle --- the motion-resistance bias the throttle observer absorbs.}
    \label{fig:pred}
  \end{center}
\end{figure}
...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.98\textwidth]{teleop_counterfactual_clearance.png}
    \figcaption{Counterfactual replay on recorded human G29 drives (five convoy
      ...
      clearance ($0.15\!\to\!4.42$~m), without fighting a capable driver.}
    \label{fig:cf}
  \end{center}
\end{figure}
...
\begin{figure}[t]
  \begin{center}
    ...
    \figcaption{The operator's view: the Chrono::Sensor driver POV over the
      deformable terrain and boulder field, with the live HUD (steering wheel and
      throttle/brake bar, lower right) and the terrain- and latency-aware
      forward-collision-warning overlay. The \textsc{red} banner and readouts ---
      time-to-collision, clearance vs.\ required stopping distance, and the
      terrain-conditioned brake budget $a_b(\hat n)$ --- are live outputs of the
      deployed warning system (soft clay, 150~ms link).}
    \label{fig:ui}
  \end{center}
\end{figure}
