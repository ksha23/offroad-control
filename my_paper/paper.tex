...
crosstrack error by $39$--$41\,\%$ relative to SCM-calibrated Pacejka and
...
replay attributes $36$ of $45$ averted collisions to the filter under
...
the same benchmarking harness, so the main ablations are comparable within
one controlled experimental matrix.
...
messages over ROS~2 (DDS) topics, integrated the Chrono-native way
through Chrono::ROS: the sim publishes the chassis body
...
and a lifecycle status message. (A self-contained ZeroMQ transport is
retained as a fallback for deployments without ROS~2.) The control
...
depth~1, matching the ZeroMQ conflating socket), which would drop a
...
\begin{figure*}[t]
  ...
  \caption{Shared-control framework. Yellow nodes are in-process
    ...
    runtime data flow over the ROS~2 / Chrono::ROS transport (ZeroMQ
    fallback); dashed arrows are the
...
median RMS CTE by $39$--$41\,\%$ relative to the SCM-calibrated Pacejka
...
sand (sand speed ratio $0.60\!\to\!0.64$), a small gain on clay
($0.53\!\to\!0.54$), and a small gain on dirt ($0.82\!\to\!0.85$),
but it \emph{cannot replace} the observer: the DOB
is higher on every terrain (clay $0.70$ vs.\ $0.54$, sand $0.67$ vs.\ $0.64$) because
...
cost a teleoperator would feel. We compare the deployed DOB-CBF against no
...
workers are isolated by a per-run ROS~2 domain (and, on the ZeroMQ
fallback, a bind-retry with linear backoff that eliminates TIME\_WAIT
races under rapid run cycling), and the publication step matches result
folders by exact timestamp so that closely-named sweep variants are
never conflated.
