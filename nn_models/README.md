...
| `vehicle_static_32_16_lhs/` | Static-features vehicle surrogate; retained because §III and the force-RMSE-vs-CTE analysis reference it. |
...
polished repo. Recover them from git history.

Not present here (not referenced by the runtime or any paper result):
...
backend in `ukf_paper_validation.py` are also not present.
